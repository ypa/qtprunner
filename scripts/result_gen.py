...
def update_action_status(db_loc, fail_lst):
    """this function will update the table with all failed actions 
    that's in the fail_lst list
    """
    conn = sqlite3.connect(db_loc)
    c = conn.cursor()
    for fail in fail_lst:
        action_name = fail.split(' [')[0]
        c.execute("update actions set status = 'failed' where action_name = ?", [action_name])
        print action_name
    conn.commit()
    c.close()
...
if __name__ == "__main__":
    if wait_till_completed(db_loc): # run only if all completed (true.)
        ...
        update_action_status(db_loc, failActs) #update database with failed actions
